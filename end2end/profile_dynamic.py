...
def process(video_path, used_model, use_gt=False):
    ...
    def traverse(depth, start, end):
        ...
        if (util.sample_lower_bound_v2(level_accuracy_std,
                                       len(used_model)) <= util.chunk_sample and
                perf_benefit < cost) or (abs(end - start) // 2) < util.small_chunk:
            sel_plan = max(sample_plan)

            k = util.sample_lower_bound_v2(level_accuracy_std, len(used_model))
            ...
            # print('[({} - {}) - {}]'.format(start, end, sel_plan))

            # with open('chunk.log', 'a') as f:
            #     f.write('{}\n'.format(end - start))
            #     f.flush()
...
def main():
    ...
    open('dynamic_breakdown.log', 'w').close()
    open('chunk.log', 'w').close()
    open('k.log', 'w').close()
    ...
    for path in util.read_path():
        ...
        if best_acc > util.majority:
            ...
            opt_t = sum([tmp[2] for tmp in tmp_res]) / 5
            exec_t = sum([tmp[3] for tmp in tmp_res]) / 5
            res += [(speed, f1, opt_t, exec_t)]
            print('-' * 73)

    with open('dynamic_breakdown.log', 'a') as f:
        for r in res:
