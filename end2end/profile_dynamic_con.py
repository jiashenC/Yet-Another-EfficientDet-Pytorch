...
def process(video_path, used_model, use_gt=False):
    ...
    agree = 0
    for k in range(length):
        if util.evaluate(cls, count, gt[str(k)]) == util.evaluate(cls, count, exec_res[str(k)]):
            agree += 1
    ...
    ) if precision != 0 and recall != 0 else 0, agree * 1.0 / length
...
def main():
    ...
    for path in util.read_path():
        ...
        if best_acc > util.majority:
            ...
            con = sum([tmp[2] for tmp in tmp_res]) / 5
            res += [(speed, f1, con)]
            print('-' * 73)
