...
def process(video_path, used_model, chunk_size=60, use_gt=False):
    ...
    for i in range(0, length, chunk_size):
        ...
        if i == 0:
            ...
            used_model = util.best_n(est_list, n=2)
        ...
        for k in range(start, end, span):
            ...
            for m in range(7, -1, -1):
                ...
                if m != 7:
                    reward = util.evaluate(cls, count, res[m][str(k)]) and util.evaluate(cls, count, gt[str(k)])
        ...
        sel_span = len(sample_plan) // 2
        tmp_sel_plan = []
        for i in range(0, len(sample_plan) - sel_span):
            tmp_sel_plan.append(max(sample_plan[i:i+span]))
        sel_plan = util.find_majority(tmp_sel_plan)[0]
