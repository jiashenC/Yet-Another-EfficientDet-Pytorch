...
def process(video_path, used_model, use_gt=False):
    ...
    def traverse(depth, start, end):
        ...
        if depth == 1:
            ...
            used_model = util.best_n(est_list, n=7) + [7]
        ...
        if (util.sample_lower_bound_v2(level_accuracy_std,
                                       len(used_model)) <= util.small_chunk and
    ...
    return exec_sample_time, exec_sample_time / exec_time


def main():
    res = []
    open('dynamic_noprune.log', 'w').close()
    ...
    for path in util.read_path():
        ...
        if best_acc > 0.1:
            ...
            over_t = sum([tmp[0] for tmp in tmp_res]) / 5
            over_t_ratio = sum([tmp[1] for tmp in tmp_res]) / 5
            res += [(over_t, over_t_ratio)]
            print('-' * 73)

    with open('dynamic_noprune.log', 'a') as f:
        for r in res:
            f.write('{},{}\n'.format(r[0], r[1]))
        f.flush()
    f.close()
